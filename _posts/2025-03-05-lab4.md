...
输入`Test a Test oslab 2024 0xadc`后，屏幕上输出为`Ret: 4; a, oslab, 2024, adc.`
...
你需要基于信号量机制解决进程同步问题。以下是具体要求和实现步骤：
...
> ##### 注意
>
> 在完成 `lab4.3` 实验后，请删除 `lab4.2` 中父进程的 `exit()` 操作，以确保代码符合实验要求。
{: .block-danger}
...
- 信号量`CountMutex`，控制对读者计数的互斥修改，初始化为1，只允许一个线程修改`Rcount`计数
