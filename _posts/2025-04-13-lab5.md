...
![no pic]({{site.baseurl}}/assets/lab5system-open-file-table.png)
...
**stat**为Linux提供的系统原语，其用于通过文件名filename获取文件信息，并保存在buffer所指的结构体stat中
...
本实验只考虑记录部分文件信息到stat结构体中，主要用于实现后续的 `find` 函数功能：
