...
> ##### 选做
>
> 本次实验有一项选做内容：完成一个类似Unit/Linux的系统调用`wait`
> 
> - 我们需要封装一个`wait()`库函数，使得测试脚本的wait测试运行
...
其中`fork`、`exit`、`getpid`等都是以前没有涉及的函数，所以，第一步就是在`lab3/lib/syscall.c`中补上相应的库函数
...
内核在链接时通过`-Ttext 0x100000`指定程序的起始地址，同时在加载时将内核进程加载到内存的`0x100000`处，设置GDT中内核代码段和数据段的基址为`0x0`。
...
框架代码的IDLE初始化过程相对简单，我们可能需要根据具体的需求，修改IDLE的初始化

> ###### Tip
>
> 还记得系统启动的执行流吗？执行流在`bootloader`中加载并跳转到内核，然后执行一系列的初始化工作；等到初始化结束后将会打开中断，此时执行流摇身一变，成为了实验中的IDLE进程，等待中断的到来
{: .block-tip}
...
由于 `sleep` 操作可以转化为时间片用完的情况，因此时钟中断处理函数 `timerHandle` 是实现进程切换的理想场所。时钟中断的核心功能包括：
...
> ##### Tip
>
> `initProc`中有初始化`pcb[0]`和`pcb[1]`的经验可供参考，**难度4**
{: .block-tip}

### sysSleep

将当前的进程的`sleepTime`设置为传入的参数，将当前进程的状态设置为`STATE_BLOCKED`，然后利用
