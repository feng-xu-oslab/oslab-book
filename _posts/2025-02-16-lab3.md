...
void initIdle(void){
	...
	if(ret == 0) {
		...
		*/
		...
		// LOG: enter idle
		char buffer[36] = "First fork: First entering the idle process\n";
		syscall(0, 0, (uint32_t)buffer, 35, 0, 0);
		// Infinite idle loop
		while(1)
		;
			// waitForInterrupt();
...
在这里，使用到了`fork`系统调用和`exec`系统调用。我们需要参考`fork`和`exec`系统调用的使用实例，完成他们

> ###### Tip
>
> 还记得系统启动的执行流吗？执行流在`bootloader`中加载并跳转到内核，然后执行一系列的初始化工作；等到初始化结束后将会打开中断，此时执行流摇身一变，成为了实验中的`IDLE`进程，等待中断的到来
{: .block-tip}
